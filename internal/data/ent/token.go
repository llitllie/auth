...
func (e TokenEdges) OwnerOrErr() (*Account, error) {
	if e.loadedTypes[0] {
		if e.Owner == nil {
...
func (*Token) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case token.ForeignKeys[0]: // account_tokens
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case token.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				t.account_tokens = new(int)
				*t.account_tokens = int(value.Int64)
			}
		}
	}
	return nil
...
func (t *Token) QueryOwner() *AccountQuery {
